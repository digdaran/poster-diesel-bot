...
|---|---|---|---|
| `backend` | `docker/backend.Dockerfile` | FastAPI + Uvicorn | REST API панели, фоновые задачи (сверка платежей по банковской выписке, освобождение просроченных резервов), `/metrics`. Вебхуки банков-эквайеров удалены вместе с интернет-эквайрингом (DECISIONS_LOG.md №44) — подтверждение оплаты только через сверку выписки. |
...
`ManualRegistration.payment_method` (`CASH`/`CASHLESS`, дефолт `CASH`) — по просьбе покупателя оператор может сформировать QR с реквизитами вместо приёма наличных (`generate_manual_registration_qr`, только для `PENDING`, идемпотентно). Номер счёта берётся из ТОГО ЖЕ счётчика `Giveaway.next_payment_number`, что и у онлайн `Payment` (§3), поэтому `PREFIX-NNNNN` остаётся глобально уникальным независимо от источника. Само подтверждение/выдача номерков не меняется — тот же `confirm_manual_registration`, что и для наличных; `payment_method` влияет только на разметку выручки в отчётах и на то, что фоновая сверка (§7.2) дополнительно подхватывает такие регистрации как safeguard.

Формирование QR также продлевает `TicketPool.reserved_until` резерва этой регистрации до `REQUISITES_INVOICE_TTL_DAYS` (`ticket_pool_repo.extend_reservation`) — короткий `MANUAL_RESERVATION_TTL_SEC` (по умолчанию ~час, рассчитан на "покупатель стоит у оператора") недостаточен для банковского перевода. `switch_manual_registration_to_cash` укорачивает резерв обратно до `MANUAL_RESERVATION_TTL_SEC`, если покупатель передумал платить по QR — иначе номерки простаивали бы замороженными на весь долгий QR-TTL (DECISIONS_LOG.md №64).
...
**Аннулирование уже завершённой покупки Super Admin'ом (обратный переход `issued -> free`, DECISIONS_LOG.md №69).** `payment_service.refund_payment`/`manual_registration_service.refund_manual_registration` — не путать с `cancel_payment`/`cancel_manual_registration` (те работают только с ещё неоплаченным `PENDING`, освобождая `reserved`-резерв). Здесь наоборот: платёж/регистрация уже `SUCCEEDED`/`CONFIRMED`, номерки уже выданы (`Ticket` создан, пул `issued`) — новый статус `REFUNDED`. Атомарный условный `UPDATE ... WHERE status = 'SUCCEEDED'/'CONFIRMED'` (тот же паттерн, что и `finalize_payment`/`cancel_payment`), затем в той же транзакции: удаление строк `Ticket` покупки (обязательно — `Ticket.pool_id`/`(giveaway_id, number)` уникальны, повторная выдача того же физического номера позже требует новой строки) и `ticket_pool_repo.release_issued_tickets` (симметрично `issue_reserved`, но `issued -> free`, не трогает `Ticket`). Деньги возвращаются вручную вне системы — сюда не входит. Доступно только через `Permission.PURCHASE_REFUND` (только Super Admin), только для завершённой покупки целиком, с обязательной причиной (`AuditLog.details`), с авто-уведомлением участника (`notification_service.notify_purchase_refunded`). `report_service`/`dashboard.py` фильтруют выручку по `SUCCEEDED`/`CONFIRMED`, поэтому `REFUNDED` авто-исключается из отчётов без отдельных правок там.
...
- Рассылки (`broadcast_service`) идут только через Telegram (продуктовое решение), реальная отправка
  через `channel_delivery_queue.send_or_enqueue` (та же гарантированная доставка, что у QR/уведомлений)
  фоновой задачей (`BackgroundTasks`) после `POST /broadcasts/{id}/send` — HTTP-ответ не блокируется
  на время рассылки (DECISIONS_LOG.md №79). Реактивная доставка (ответ в чат покупателя, напр. `_deliver_tickets` в каждом `channels/*/handlers.py`) идёт через тот канал, которым участник совершал покупку; проактивные уведомления backend (webhook банка, фоновая сверка) уходят через `app/services/notification_service.py::_resolve_notify_targets` во ВСЕ подходящие привязки получателя одновременно (Telegram и VK, если обе есть) — VK участвует, только если участник явно разрешил сообщения от сообщества (см. §7.1, DECISIONS_LOG.md №43, отменяет "один канал" из №33).
- **Доставка через мессенджер-каналы гарантированная по каждому каналу** (QR при создании счёта — `channels/*/handlers.py`; проактивные уведомления — `notification_service.py`): единая точка входа `app/services/channel_delivery_queue.py::send_or_enqueue` сперва пытается отправить немедленно (`app/channels/retry.py::send_with_retry`, до 3 попыток с backoff, ограничено `asyncio.Semaphore` в `TelegramChannel`/`VkChannel` на процесс — `CHANNEL_SEND_CONCURRENCY_LIMIT`), а при исчерпании ставит `PendingChannelDelivery(status=PENDING)` в `pending_channel_deliveries`, докручиваемую `channel_delivery_queue.process_pending_deliveries` на каждом тике `run_background_loop` до успеха, без ограничения числа попыток (см. DECISIONS_LOG.md №72/№73).

### 7.1. VK-адаптер (`channels/vk/`)

Реализован на `vkbottle` (Bots Long Poll API, без Callback API/вебхука — та же топология "канал = отдельный процесс", что и у `channel-telegram`); активен в проде (`ACTIVE_CHANNELS`, см. DECISIONS_LOG.md #32/#33). Структура `channels/vk/` (`dispatcher.py`/`handlers.py`/`state.py`/`main.py`) параллельна `channels/telegram/`.
...
- Медиа — upload-флоу VK (`photos.getMessagesUploadServer`/`saveMessagesPhoto` через `PhotoMessageUploader`), кэш attachment-строки в `GiveawayPoster.media_cache` (перенесено из прежнего `Giveaway.poster_media_cache` при переходе на несколько постеров на розыгрыш, см. DECISIONS_LOG.md №46).
...
- Деплой — процесс `channel-vk` (`docker/vk.Dockerfile`), extras `vk`. Backend дополнительно поднимает свой outbound-only `VkChannel` (без polling, `backend/main.py::lifespan`) для проактивных уведомлений — тем же способом, что и `TelegramChannel` (DECISIONS_LOG.md #24) — поэтому `docker/backend.Dockerfile` тоже ставит `[vk]`-зависимости (см. DECISIONS_LOG.md #25 про инцидент с забытыми зависимостями канала в образе backend).
...
**Подтверждение оплаты — сверка выписки расчётного счёта, не вебхук.** `app/services/bank_reconciliation_service.py::reconcile()` (вызывается фоновым циклом `backend/background/__init__.py::run_background_loop` наравне с `_reconcile_pending_payments`, но независимо от него — см. §3): раз в тик забирает входящие операции по счёту за скользящее окно (`BANK_STATEMENT_LOOKBACK_DAYS`) через `BaseBankStatementProvider` (`app/payments/bank_statement.py`, реализация — `TBankStatementProvider` поверх Т-Банк T-API `GET /api/v1/statement`, по документации — тот же принятый на старте риск, что и у интернет-эквайринга до его удаления, см. DECISIONS_LOG.md №1/№40) и находит все операции, совпадающие с неоплаченным `Payment(provider=REQUISITES_QR)` **по назначению платежа** (префикс розыгрыша + номер счёта, `Giveaway.prefix` уникален глобально, `find_matching_entries`; поиск нечувствителен к регистру, кириллическим двойникам латиницы, виду разделителя и ведущим нулям, с границами с обеих сторон номера; `TBankStatementProvider` склеивает все поля назначения операции (`payPurpose`, `description`, …), а не берёт первое — см. DECISIONS_LOG.md №84). Их суммы **суммируются** и сравниваются с `Payment.amount` (несколько частичных переводов по одному QR складываются, а не ищется одна точно совпавшая операция — см. DECISIONS_LOG.md №38/№53): сумма ≥ ожидаемой закрывает счёт (`payment_service.finalize_payment(SUCCEEDED)`) всегда, включая переплату (без возврата разницы — вне ТЗ §21); сумма < ожидаемой оставляет счёт `PENDING`. Расхождение в обе стороны логируется (`bank_statement_amount_mismatch`) и сохраняется на `Payment.amount_mismatch`/`amount_mismatch_bank_amount` (теперь означает "сумма ВСЕХ подходящих операций", не одной) для подсветки в панели «Продажи» — переплата и недоплата различаются разными бейджами (`SalesPage.tsx`, сравнение с `Payment.amount`); пока недоплата активна, TTL-автопросрочка для этого счёта пропускается. `RequisitesQrProvider.check_status` (разовая проверка по кнопке участника) использует ту же логику суммирования, но не пишет `amount_mismatch` сам — это исключительно забота фонового тика. Неоплаченные счета старше `REQUISITES_INVOICE_TTL_DAYS` (и без активной недоплаты) помечаются `FAILED` (резерва нет — освобождать нечего, только уменьшается сумма в лимите ожидающих экземпляров участника, см. DECISIONS_LOG.md №45).

Тот же тик `reconcile()` дополнительно подбирает `PENDING` `ManualRegistration` с `payment_method=CASHLESS` (QR, сформированный оператором, см. §4) и подтверждает совпавшие через `manual_registration_service.confirm_manual_registration` — safeguard на случай, если оператор забыл нажать «Подтвердить» сам после того, как деньги пришли. В отличие от `Payment`: нет lookback по `CANCELLED`/`FAILED` (просроченные `PENDING` уже забирает `_release_expired_manual_registrations`, отдельный шаг фонового цикла) и нет отдельных полей/UI-подсветки расхождения суммы — недоплата просто остаётся `PENDING` до ручного разбора (см. DECISIONS_LOG.md №62).

**Статус фоновой сверки виден в панели.** Каждый вызов `reconcile()` — включая случай "нет неоплаченных счетов requisites_qr" (запрос к банку не делался вовсе) — пишет строку `BankReconciliationRun` (`app/models/bank_reconciliation_run.py`, `_record_run()`): статус (`SUCCESS`/`FETCH_FAILED`), сколько счетов проверено, сколько операций пришло в выписке, сколько совпадений/расхождений/TTL-просрочек, сколько раз индивидуальная финализация упала исключением. Старые строки чистятся инлайн (`BANK_RECONCILIATION_RUN_RETENTION_DAYS`, по умолчанию 30 дней) при каждой записи — без отдельного cron. Отдельная агрегация `get_payments_brief()` считает счета `requisites_qr` по дню СОЗДАНИЯ (`Payment.created_at`, UTC-сутки) и раскладывает на всего/успешно/ожидают/спорные (последнее — `PENDING` с активным `amount_mismatch`) для сегодня и вчера. `GET /api/bank-reconciliation/status` (`backend/api/bank_reconciliation.py`, `Permission.VIEW_BANK_RECONCILIATION` — только Administrator/Super Admin) отдаёт и то, и другое одним запросом. Фронтенд (`BankReconciliationStatusPanel.tsx`) — тонкая строка на «Продажи» (здоровье процесса + бриф сегодня/вчера) с кнопкой, открывающей модальное окно с историей 20 тиков и агрегатами за 24ч; не отдельный пункт меню (см. DECISIONS_LOG.md №48, №49).

**Квитанции.** Участник присылает в бот фото/документ квитанции ПОСЛЕ оплаты — `app/services/receipt_service.py::save_receipt` сохраняет файл на диск (`RECEIPTS_DIR`, тот же bind-mount паттерн `./data/`, что и БД) и создаёт `PaymentReceipt`. Привязка к конкретному `Payment`: если участник только что выбрал счёт в разделе «Мои покупки» (кнопка «📎 Прислать квитанцию» → `select_payment_receipt:{payment_id}` в Telegram / VK-payload `{"a": "select_payment_receipt", "id": ...}`, id счёта хранится в данных FSM-состояния под ключом `receipt_payment_id`, состояние `PurchaseStates.awaiting_receipt`/`AWAITING_RECEIPT`) — квитанция привязывается именно к нему; иначе (файл прислан без предварительного выбора, как и раньше) — к последнему `PENDING`-платежу участника (fallback, сохранён ради обратной совместимости). Не распознаётся — только хранится, просматривается оператором в панели («Продажи» → колонка «Квитанция»). Раздел «Мои покупки» (`on_my_tickets`, оба канала) дополнительно показывает список PENDING-счетов участника (`payment_service.list_pending_payments`) с суммой, номером счёта, признаком «квитанция получена/не прислана» и сроком действия (`expires_at` = `Payment.created_at` + `REQUISITES_INVOICE_TTL_DAYS`, тот же TTL, что использует `bank_reconciliation_service.reconcile()` для аннулирования — см. DECISIONS_LOG.md №68); для счетов без квитанции — инлайн-кнопка выбора (см. DECISIONS_LOG.md №51).
...
- Все чувствительные рантайм-данные (БД, бэкапы, квитанции участников, TLS-сертификаты/ACME-аккаунт Caddy) — bind mount в `./data/` внутри репозитория (не именованные Docker volumes), ради переносимости: копия каталога проекта содержит всё состояние для запуска на другом хосте (см. DECISIONS_LOG.md #28). `./data/` в `.gitignore`. Квитанции (`RECEIPTS_DIR=/data/receipts`) и цифровые постеры розыгрышей (`POSTER_DIR=/data/posters`, загружаются через веб-админку, см. DECISIONS_LOG.md №46) смонтированы во все три процесса, пишущих/читающих их напрямую через `app/` (backend, channel-telegram, channel-vk) — тот же принцип, что и для `DATABASE_PATH`.
- Caddy: HTTPS (ACME или self-signed для dev), IP-whitelist для панели и API (`frontend`/`backend`) — наружу без ограничения по IP ничего не публикуется, банковских webhook-эндпоинтов нет (интернет-эквайринг удалён, см. DECISIONS_LOG.md №44; оплата подтверждается сверкой выписки), `/metrics` не проксируется.
- `scripts/backup_db.sh` — `sqlite3 $DB "VACUUM INTO '$BACKUP'"`, gzip, ротация по `BACKUP_RETENTION_DAYS`.
- `scripts/full_backup.sh` — почасовой полный офсайт-бэкап через `restic` (дедупликация + шифрование): свежий снимок `backup_db.sh` + `data/receipts`, `data/posters`, `data/caddy`, `.env` → локальный repo (`RESTIC_LOCAL_KEEP_WITHIN`, по умолчанию 1 день) и repo на другом сервере по SSH/SFTP (`RESTIC_REMOTE_REPO`, `RESTIC_REMOTE_KEEP_HOURLY`/`RESTIC_REMOTE_KEEP_DAILY`). Исходный код не копирует — он уже в git. См. LOG №66.
- `scripts/deploy.sh` — `git pull`, `docker compose build`, `docker compose up -d`, ожидание healthcheck.
- `ssh-tunnel` (sidecar-контейнер, `docker/ssh-tunnel.Dockerfile`) — `autossh -N -D` поднимает динамический SOCKS5-форвардинг по SSH (ключевая авторизация, новая ed25519-пара) на удалённый хост с доступом к `api.telegram.org`; `channel-telegram` ходит в Bot API через `socks5://ssh-tunnel:$SSH_TUNNEL_LOCAL_PORT` вместо стороннего прокси-провайдера — заменяет прежний `TELEGRAM_PROXY_URL`, указывавший на внешний прокси. Выбран именно динамический форвардинг (`-D`), а не проброс порта 443 напрямую (`-L`), чтобы TLS/SNI handshake оставался с настоящим `api.telegram.org` (сертификат туда и выдан) — через SSH-туннель идёт только TCP-транспорт. Приватный ключ и `known_hosts` — bind mount из `./data/ssh/` (тот же принцип переносимости, что и остальной `./data/`, не коммитятся), `StrictHostKeyChecking=yes` без исключений. См. DECISIONS_LOG.md №71, README.md (раздел деплоя).
