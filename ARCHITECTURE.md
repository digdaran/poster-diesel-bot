...
|---|---|---|---|
| `backend` | `docker/backend.Dockerfile` | FastAPI + Uvicorn | REST API панели, webhook-роутеры банков, фоновые задачи (сверка платежей, освобождение просроченных резервов, экспорт номерков в Google Sheets), `/metrics`. |
...
**Экспорт номерков в Google Sheets** (вне ТЗ, по прямому запросу заказчика, см. DECISIONS.md №44). `app/services/sheets_export_service.py::sync_all_giveaways` — ещё один независимый шаг того же фонового цикла (`run_background_loop`), но на собственном интервале (`GOOGLE_SHEETS_SYNC_INTERVAL_SEC`, отдельном от `ONLINE_STATUS_POLL_INTERVAL_SEC`), а не событие внутри резервации пула — внешний сетевой вызов к Google API не должен попадать в критичный `BEGIN IMMEDIATE`-участок (§3). Для каждого розыгрыша со указанным `Giveaway.google_sheet_id` (своя таблица на розыгрыш — ссылка публичная, общий лист на все розыгрыши раскрыл бы чужие) читает все `Ticket` розыгрыша и полностью перезаписывает диапазон `A1` (номерки не отзываются в этой версии — список только растёт, полная перезапись не теряет данных). Телефон участника маскируется (`app/core/phone.py::mask_phone`, видны последние 5 цифр) — таблица открыта участникам по ссылке, полный телефон/ФИО туда не идут. Аутентификация — файл ключа сервисного аккаунта (`GOOGLE_SHEETS_CREDENTIALS_FILE`), которому администратор должен вручную выдать доступ редактора к каждой таблице в Google; платформа доступ не запрашивает.
