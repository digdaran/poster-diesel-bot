...
маппинг статусов, формат чека (Receipt, см. `TBankProvider._build_receipt` —
у ВТБ Эквайринга свой формат объекта чека, но тот же набор входных данных
`PaymentOrder`: `unit_price`/`quantity`/`participant_phone`). Структура
(аргументы, форма ответа) — по аналогии с TBankProvider, чтобы подключение
свелось к реализации трёх методов ниже.
"""
