...
В отличие от интернет-эквайринга (удалён, см. DECISIONS_LOG.md №44): нет сетевого
вызова при создании платежа (`create_payment` — чистая сборка QR-payload), нет
банковской сессии для отмены (`cancel` — no-op), нет вебхука
(`verify_and_parse_webhook` поднимает ошибку — не используется). Подтверждение
оплаты идёт асинхронно через
