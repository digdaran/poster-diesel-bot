...
def release_issued_tickets(
    session: Session,
    *,
    payment_id: int | None = None,
    manual_registration_id: int | None = None,
) -> list[TicketPool]:
    """Возвращает УЖЕ ВЫДАННЫЕ (`issued`) номера обратно в `free` — используется
    при аннулировании супер-админом уже завершённой (оплаченной/подтверждённой)
    покупки постфактум (см. DECISIONS.md, DECISIONS_LOG.md №69). Не путать с
    `release_reservation` — та освобождает ещё не выданный резерв (`reserved`)
    неоплаченного счёта.

    Как и `issue_reserved`, не трогает строки `Ticket` — это ответственность
    вызывающего сервиса (нужен доступ к списку кодов до их удаления).
    """
    if (payment_id is None) == (manual_registration_id is None):
        raise ValueError("Нужно передать ровно одну из payment_id / manual_registration_id")

    filters = [TicketPool.status == TicketPoolStatus.ISSUED]
    if payment_id is not None:
        filters.append(TicketPool.payment_id == payment_id)
    else:
        filters.append(TicketPool.manual_registration_id == manual_registration_id)
    ...
    session.execute(
        update(Giveaway)
        .where(Giveaway.id == giveaway_id)
        .values(tickets_issued=Giveaway.tickets_issued - len(ids))
    )
    session.flush()
    return list(session.execute(select(TicketPool).where(TicketPool.id.in_(ids))).scalars().all())
